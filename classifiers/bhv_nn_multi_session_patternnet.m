...
Trials = cf(@(Trial)  MTATrial.validate(Trial), get_session_list(sessionList));
...
[states,stcMode,featureSet,sampleRate,model,nNeurons,...
 ...
 targetState,prctTrain,normalizationSessionList,referenceTrial] = ...
    DefaultArgs(varargin,defArgs,'--struct');


% LOAD the feature set
features = cf(@(Trial,fetSet,sr) feval(featureSet,Trial,sr,false),...
               Trials,...
              repmat({featureSet},1,numel(Trials)),...
              repmat({sampleRate},1,numel(Trials));
...
if map2reference,
    ...
    cf(@(f,t,r) f.map_to_reference_session(t,r),...
       features, Trials, repmat({refSession},1,numel(Trials)));

             cf(@(f,t,r) f.map_to_reference_session(t,r)    ,features,Trials,...
            repmat({param.referenceTrial},1,numSessions));
for s = 1:numSessions, fet{s}.data(~nniz(xyz{s}),:,:) = 0;end


    if normalize,
        if isempty(normalizationSessionList),
