function [Stc] = optimize_stc_transition_eduardo2(Trial,varargin)
...
for s = 1:nsts;
    ...
    % Correcting gaps with extra gaussian
NewConv=zeros(length(xyz.data),nsts);
for s = 1:size(BinaryConv,2);
    gauss=[];
    gauss = normpdf(linspace(0,2,round(Samples(s)*1.4)),1,2);
    gauss=gauss./sum(gauss);
    NewConv(:,s)=conv(double(BinaryConv(:,s)),gauss,'same');
end    
    
% $$$ NewBinaryConv = bsxfun(@rdivide,NewConv,max(NewConv,[],1))>.5;
% $$$ overlaps2 = sum(NewBinaryConv,2)>1;
% $$$ NewBinaryConv(overlaps2,:)=0;

[~,mind] = max(bsxfun(@rdivide, NewConv,max(NewConv)),[],2);

smat = zeros([size(xyz,1),nsts])';
smat([mind+(0:size(xyz,1)-1)'*nsts]) = 1;
smat = smat';
smat(sum(NewConv,2)<=0,:)=0;
smat = logical(smat);
...
Stc.updateMode([Stc.mode '_OPTE2']);
Stc.save(1);
