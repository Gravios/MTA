function [mxr,mxp,fo] = minRate(Pfs,varargin)
% function [mxr,mxp] = minRate(Pfs,varargin)
% return the minimum firing rate and location of a unit(s)
%
%  IN:
%
...
% $$$         [mxr(u==units),mxp(u==units)] = min(rateMap.*mask);
      ...
      case 'COM' % center of mass
                 % select bins near min := < sigThresh (default: 200mm)
% $$$         rateMap = Pfs.data.rateMap(:,Pfs.data.clu==u,1);
% $$$         if size(rateMap,2)==0,  rateMap = nan([size(Pfs.data.rateMap,1),1]);  end        
% $$$         [~,mxp(u==units)] = min(rateMap.*mask);
        % mxp = 
