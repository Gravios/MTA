function [spkv,statesSpk] = get_unit_spkVar(Trial,units,varargin)
...
[statesSpk,distThresh,sampleRate,sigma] = DefaultArgs(varargin,defargs,'--struct');
%---------------------------------------------------------------------------------------------------
...
% TAG creation -------------------------------------------------------------------------------------
if isempty(tag),
    tag = DataHash({statesSpk,sigma,distThresh,sampleRate});
end
%---------------------------------------------------------------------------------------------------


% MAIN ---------------------------------------------------------------------------------------------
filename = fullfile(Trial.spath,[Trial.name,mfilename,'-',tag,'.mat']);
...
if ~exist(filename,'file') || overwrite,
    ...
    hrz = compute_hrz(Trial,units,pft,'sampleRate',sampleRate);
    ghz = compute_ghz(Trial,units,pft,'sampleRate',sampleRate,'sigma',sigma);
    ...
    for unit = 1:numel(units);
        ...
        spkmap = cat(1,spkmap,[ones([numel(res),1]).*tind,ones([numel(res),1]).*units(unit)]);
        spkego = cat(1,spkego,multiprod(bsxfun(@minus,mxp,sq(xyz(res,'hcom',[1,2]))),...
                                        hvec(res,:,:),2,[2,3]));
