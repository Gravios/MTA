...
switch feature
  case 'Head Height'
    vel = MTADxyz('data',log10(xyz(:,'head_front',3)),'sampleRate',xyz.sampleRate);
    vnn = nniz(vel);
...
srv = nan(numel(N),rhm.size(2));;
for f =1:rhm.size(2),
    mrv(:,f) = accumarray(vbs(nniz(vbs)),srhm(nniz(vbs),f),[vbins,1],@nanmean);
    srv(:,f) = accumarray(vbs(nniz(vbs)),srhm(nniz(vbs),f),[vbins,1],@nanstd);
end
mrv(N<30,:) = nan;
srv(N<30,:) = nan;
