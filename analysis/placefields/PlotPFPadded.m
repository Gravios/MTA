function [RateMap, Bins, SI, Spar] = PlotPFPadded(Session,spkpos,pos,binDims,SmoothingWeights,type,bound_lims,posSampleRate)
...
if ndims==1,
    ...
    amsize = msize'+round(SmoothingWeights.*12).*2;
end

% ACCUMULATE feature space occupancy in seconds
Occupancy = accumarray(bsxfun(@plus,pos,round(SmoothingWeights*12)),1,amsize)./posSampleRate;
...
if ~isempty(spkpos),
    ...
    SpikeCount = accumarray(bsxfun(@plus,spkpos,round(SmoothingWeights*12)),1,amsize);
else
...
sind = cell(1,ndims);
for i = 1:ndims,
    sind{i} = linspace(-round(amsize(i)/2),round(amsize(i)/2),amsize(i));
end
[sind{:}] = ndgrid(sind{:});
for i = 1:ndims,
