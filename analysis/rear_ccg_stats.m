...
for si=1:length(slist);
    ...
    for s = 1:length(bhvs),
        ...
        for unit = 1:size(pfs.cluMap,1),
            try
                pfMaxPos{s}(unit,:) = pfs.maxRatePos{unit}(pfs.maxRateMax{unit},:);
            end
        end
    end
    ...
    for s = 1:length(bhvs),
        for j = 1:length(bhvs),
            ...
            MTAccg(Session,...
                   [bhvs{s}_pfc_],...  NAME
                ['ccg at ' bhvs{s} ' onset and offset filtered: id3,d1.5'],... DESCRIPTION
                {bhv_on,bhv_off},...
                {[bhvs{s} '_onset'],[bhvs{s} '_offset']},
                [],...         Units to be processed ([] defaults to all)
                true,...       overwrite previously calculated values 
                true,...       add a six digit random number to the filename
                'abs_dist',... method used during partitioning
                4,...          number of partitions
                pfMaxPos{j},...   partition feature used to distribute events into partitions
                1,...          keep a record of the xy position of the events
                0,...          surrogate sample size when selcting from a periods
                1,...          number of iterations
                binSize,...    100ms time bins
                halfBins,...   30 bins on each side of the event (61 total)
                normalization);% rates are normalize to Hz
        end

        MTAccg(Session,...
           [bhvs{s} 'pfc_surrogate'],...  NAME,
           ...
           'abs_dist',...    method used during partitioning
           4,...             number of partitions
           pfMaxPos{s},...   partition feature used to distribute events into partitions
           1,...             keep a record of the xy position of the events
           round(mean([length(bhvon),length(bhvoff)])),... surrogate sample size when selcting from a periods
