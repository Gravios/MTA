function [pfs,pfr,pfa,pfd] = FS_recompute_placefields(Trial)


%'overwrite',false

% ROOM frame of reference  (X,Y)
pfsArgs = struct('states',           'theta&gper&vel-rear',        ... Computational Periods 
                 'tag',              'theta',                      ...
...
% $$$                        
% $$$ % ROOM frame of reference  (X,Y)
% $$$ pfsArgs = struct('states',           'theta&gper&vel&approach-rear',   ... Computational Periods
% $$$                  'tag',              'approach',                   ...
% $$$                  'binDims',          [20,20],                      ... Physical size of bins in milimeters
% $$$                  'SmoothingWeights', [3.5,3.5],                    ... Gaussian smoother prameters, std deviation in bins 
% $$$                  'numIter',          1,                            ... number of bootstraps
...
% $$$                        
% $$$ % ROOM frame of reference  (X,Y)
% $$$ pfsArgs = struct('states',           'theta&gper&vel&depart-rear',     ... Computational Periods 
% $$$                  'tag',              'depart',                     ...
% $$$                  'binDims',          [20,20],                      ... Physical size of bins in milimeters
% $$$                  'SmoothingWeights', [3.5,3.5],                    ... Gaussian smoother prameters, std deviation in bins 
% $$$                  'numIter',          1,                            ... number of bootstraps
