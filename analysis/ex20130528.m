...
ufridx = LocalMinima(-Trial.ufr(:,unit),1,1);

for bhv = 1:length(bhvs), 
    fet.xyz{bhv} = SelectPeriods(sq(Trial.xyz(:,Trial.Model.gmi(Trial.trackingMarker),[1,2])),Trial.Bhv.getState(bhvs{bhv}).state,'d',1,1);
