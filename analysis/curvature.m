...
sinosxy = sinuosity(Trial.xyz,[1,3,7],[1:3]);

Trial = MTATrial('jg05-20120317');
Trial.xyz.load(Trial);
Trial.xyz.filter(gausswin(91)./sum(gausswin(91)));
...
nw = numel(wins);
sxy = zeros([Trial.xyz.size([1,2]),2,nw]);

for i = 1:9,
for j = 1:nw
sxy(:,i,:,j) = circshift(Trial.xyz(:,i,[1,2])-circshift(Trial.xyz(:,i,[1,2]),-wins(j)),round(wins(j)/2));
end
end


ims = circshift(sq(sum(sxy(:,1,:,1:30).*sxy(:,m,:,1:30),3)),round(-.5*wins(end)))';
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
index = 26400:26700;
pview = zeros([Trial.xyz.size(1),1]);
pview(index) = 1;


figure,plot3(Trial.xyz(index,1,1),Trial.xyz(index,1,2),Trial.xyz(index,1,3))
hold on,plot3(Trial.xyz(index,7,1),Trial.xyz(index,7,2),Trial.xyz(index,7,3),'r')
hold on,plot3(Trial.xyz.data(index(1),7,1),Trial.xyz.data(index(1),7,2),Trial.xyz.data(index(1),7,3),'.g')
...
hold on,plot3(Trial.xyz(windex,1,1),Trial.xyz(windex,1,2),Trial.xyz(windex,1,3),'.g')
hold on,plot3(Trial.xyz(windex,7,1),Trial.xyz(windex,7,2),Trial.xyz(windex,7,3),'.g')

hold on,plot3(Trial.xyz(rindex,7,1),Trial.xyz(rindex,7,2),Trial.xyz(rindex,7,3),'.m')
hold on,plot3(Trial.xyz(rindex,1,1),Trial.xyz(rindex,1,2),Trial.xyz(rindex,1,3),'.m')
