function ant_fig_comp(Trial,varargin)
...
while 1,
    ...
    subplot2(y,x,5,2);bar(Bccg.tbin,sq(fccg(:,unit,1,:,1))),axis xy,axis tight
    title('CCG centered on rear onset')

    subplot2(y,x,6,2);bar(Bccg.tbin,sq(fccg(:,unit,2,:,1))),axis tight,axis xy
    title('CCG centered on rear offset')

    subplot2(y,x,5,3);imagesc(Bccg.tbin,0:Bccg.partitions:1000,sq(dccg(:,unit,1,:,1))'),axis xy,
    title('CCG centered on rear onset')

    subplot2(y,x,6,3);imagesc(Bccg.tbin,0:Bccg.partitions:1000,sq(dccg(:,unit,2,:,1))'),axis xy
    title('CCG centered on rear offset')
    ...
    caxis([-1,1])
    title({'log10 ratio around rearing onset','(0ms to 750ms) and (-750ms to 0ms)'});

    % Place Field: Rearing 
    subplot2(y,x,[3,4],3); 
    pf_search.type = 'xy';
    pf_search.stateLabel = 'pre_peri_rear_offset';
    Pf1 = Trial.getPfs(pf_search);
    pf_search.stateLabel = 'post_peri_rear_offset';
    Pf2 = Trial.getPfs(pf_search);
    ppf(Pf1.xbin,Pf1.ybin,log10((Pf2.rateMap{unit}+1)./(Pf1.rateMap{unit}+1)))
    caxis([-1,1])
    title({'log10 ratio around rearing offset','(0ms to 750ms) and (-750ms to 0ms)'});
    ...
    switch display_mode
      ...
      case 'report'
        ...
        reportfig(2993,[Trial.filebase '.ccg.ant_fig_comp'],[],['unit: ' num2str(unit)],[],0);
        print(2993,'-depsc2','-r200',['/data/homes/gravio/mrep/ant_figs/' ...
                            Trial.name '\' Trial.name '.ant_fig_comp.' ...
                            num2str(unit) '.eps'])
