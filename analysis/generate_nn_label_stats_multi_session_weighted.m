function [out] = generate_nn_label_stats_multi_session_weighted(mode,varargin)
...
% SETUP paralellization workers
    try, delete(gcp('nocreate')); catch err, disp(err); end
    if numel(trainingList) > 9,
        par = parpool(10);
    else,
    ...
    parfor dropIndex = 1:numel(trainingList),
        ...
        bhv_nn_multi_session_weighted_patternnet(...
    ...
    try, delete(par); catch err, disp(err); end
...
% COMPUTE ----------------------------------------------------------------------------------------
  case 'compute'
    ...
    if isempty(sessionList),
        ...
        labelingStats = repmat(struct('confusionMatrix',nan([numel(states),numel(states)]),...
                                      'precision',      nan([1,numel(states)]),...
                                      'sensitivity',    nan([1,numel(states)]),...
                                      'accuracy',       nan),...
                               [1,numel(sesList)]);
...
% SETUP paralellization workers
        ...
        if numel(sesList) > 9,
            ...
            par = parpool(numel(sesList));
        end

        parfor dropIndex = 1:numel(sesList),
        ...
        [stc,labelingStats,networkOutput] = bhv_nn_multi_session_weighted_patternnet(                ...
            sessionList,            featureSet,   states,   keys,  model,                   ...
            sampleRate,             nNeurons,     nIter,    randomizationMethod,            ...
            map2reference,          normalize,    referenceTrial,                           ...
            trainingSessionList,    normalizationSessionList,                               ...
...
% LOAD neural network labeled state collections
    ...
    if sessionListWasEmpty,
        ...
        stc = optimize_stc_transition(stsRaw.stc,                                            ...
            sessionList,            featureSet,   states,   keys,  model,                    ...
            sampleRate,             nNeurons,     nIter,    randomizationMethod,             ...
            map2reference,          normalize,    referenceTrial,                            ...
            trainingSessionList,    normalizationSessionList,                                ...
...
% FORMAT statistical data for boxplot
    dat = [cell2mat(af(@(x)x.accuracy,stsRaw.labelingStats));...
...
% FORMAT statistical data for boxplot
    dat = [cell2mat(af(@(x)x.precision,stsRaw.labelingStats));...
...
% FORMAT statistical data for boxplot
    dat = [cell2mat(af(@(x)x.sensitivity,stsRaw.labelingStats));...
