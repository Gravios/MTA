function [pfs,bins] = compute_egohvf_ratemap(Trial,units,varargin)
...
% COMPUTE head velocity in the frame of reference of the head
    hvf = fet_href_HXY(Trial,sampleRate,false,'trb',4);
    hvfBinInds = discretize(hvf(:,1),hvfBinEdgs);
...
for phase = 1:numel(phzBinCtrs)
    for hvf = 1:numel(hvfBinCtrs)    
        ...
        pargs.tag = ['egofield_theta_phase_hvf_',num2str(hvf),'_',num2str(phase),stag];        
        filepath = fullfile(Trial.spath, [Trial.filebase,'.Pfs.',pargs.tag,'.mat']);
        
        if exist(filepath,'file')
            pfs{phase,hvf} = load(filepath).Pfs;
            if overwrite,
                pfs{phase,hvf}.purge_savefile();
            else
                continue;
            end% if
        end% if exist
