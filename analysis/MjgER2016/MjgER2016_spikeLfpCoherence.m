...
% RASTER STUFF
Trial = Trials{18};    % jg05-20120312.cof.all
unitsPyr = units{18};     % good units
...
ufrNon = Trial.load('ufr',xyz,[],unitsNon,0.75);
...
specArgsGamma = struct('nFFT',2^8,...
                  ...
                  'nTapers',[],...
                  'FreqRange',[25,120]);
[gys,gfs,gts] = fet_spec(Trial,lfp,[],[],[],specArgsGamma);
...
figure,
cmapLimsTheta = [1,3;1,3;1,3.5;1,3.5;1,3.5];
cmapLimsGamma = [2.4,3.4;...
                 2.4,3.4;...
                 2.8,3.9;...
                 2.8,3.9;...
                 2.8,3.9];
sp = tight_subplot(28,1,0.0,0);
for s = 2:2:10,
axes(sp(s-1))
imagesc(gts,gfs,log10(gys(:,:,s/2))');
axis('xy');
caxis(cmapLimsGamma(s/2,:));
colormap(gca,'jet');
axes(sp(s))
imagesc(lts,lfs,log10(lys(:,:,s/2))');
axis('xy');
caxis(cmapLimsTheta(s/2,:));
colormap(gca,'jet');
end
