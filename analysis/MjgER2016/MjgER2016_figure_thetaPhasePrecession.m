...
% INDEX data
    ind = tind                                                             ...
          ...
          & spkv.ppr(:,s+1) > 0.2;
% PLOT jpdf of data
    hist2([repmat(spkv.ghz(ind),[2,1]),reshape(bsxfun(@plus,repmat(spkv.phz(ind),[1,2]),[0,2*pi]),[],1)],...
          linspace(-1,1,21),...
          linspace(-pi,3*pi,21));
    sax(end).XTickLabels = {};
    sax(end).YTickLabels = {};
...
% INDEX data
    ind = tind                                                              ...
          & any(spkv.stc(:,statesInds(s)),2)                                ...
          & ismember(spkv.map,cluSessionMapSubset_N,'rows')                 ...
          & abs(spkv.ego(:,2))<100                                          ...
          & abs(spkv.ddz(:,1))<350                                          ...          
          & spkv.ppr(:,s+1) > 0.2;
% PLOT jpdf of data
...
% $$$ %    ind = tind & any(spkv.stc(:,statesInds(s)),2) & ismember(spkv.map,cluSessionMap(genericScores{s},:),'rows');
% $$$ %    ind = tind & any(spkv.stc(:,statesInds(s)),2) & ismember(spkv.map,cluSessionMap(RmaxN(:,s+1)>0.25,:),'rows');
% $$$ % PLOT jpdf of data
...
% CREATE subplot axes
            ...
            if mean(tpp.phzRateMax(:,uind,s)) > 4,
                plot(tpp.phzRateMeanPos(:,uind,s)',            ... x = drz 
...
% $$$             [hl,hp] = boundedline(tpp.phzRateMeanPos(:,uind,s)',...
% $$$                         pftHZTPD{1}{1}.adata.bins{2}',...
% $$$                         0,...
% $$$                         ...% tpp.phzRateMeanPosBSStd(:,ismember(cluSessionMapSubset,[t,u],'rows'),s)'.*1.96./10,...
% $$$                         ['-',stsColor(s)],...
% $$$                         'alpha',...                        
% $$$                         'transparency',0.3,...
% $$$                         'orientation','horiz');
...
% $$$                         pftHZTPD{1}{1}.adata.bins{2}',...
% $$$                         0,...
% $$$                         ... %tpp.phzRateMeanPosBSStd(:,uind,s)'.*1.96./10,...
% $$$                         ['-',stsColor(s)],...
% $$$                         'alpha',...
% $$$                         'transparency',0.3,...
% $$$                         'orientation','horiz');
...
for s = 1:3;
    sax(end+1) = axes('Units','centimeters',                                             ...
                      'Position',[fig.page.xpos(xind),                                   ...
                                  fig.page.ypos(yind+s*2)-fig.subplot.height/2-         ...
                                     fig.subplot.height.*((s-1)/3),                      ...
                                  fig.subplot.width*2,                                   ...
                                  fig.subplot.height.*2],                                ...
                      'FontSize', 8,                                                     ...
                      'LineWidth',1);
