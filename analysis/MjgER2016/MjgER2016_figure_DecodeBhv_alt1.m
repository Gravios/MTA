...
d2ind =   logical(d2stcm(:,1))                                  ...
          ...
          & d2post>=0.001                                       ...
          & ismember(d2uinc,[2:16])                             ...
          & nniz(d2lonInd)                                      ...
          & nniz(d2latInd)                                      ...
          & nniz(d2phzInd);

dc2.spdBinEdges = prctile(d2spd(d2ind),[0:10:100]);
dc2.spdBinCenters = mean(cat(1,dc2.spdBinEdges(2:end),dc2.spdBinEdges(1:end-1)));
d2spdInds = discretize(d2spd,dc2.spdBinEdges);
for s = 1:numel(dc2.spdBinCenters),
    sind = d2ind & d2spdInds==s;
    subplot(10,1,s);
    hist2([d2lon(sind),d2lat(sind)],ferrorBinEdges{1},ferrorBinEdges{2});
    caxis([0,100])
end
...
for t = 1:numTrials
    ...
    disp(['[STATUS] Loaded dct decoded position: ', Trials{t}.filebase]);
end
...
load('/storage/gravio/data/project/general/analysis/MjgER2016_decodeBhv_dvars.mat');
...
sBySesHbaHvl = load('/storage/gravio/data/project/general/analysis/MjgER2016_req20191104_jpdf_2d_PS_7ebeda80dc3c482b4c8fe95ebae3b73f.mat');
...
%%%<<< phase-precession speed relationship
...
dct.spdBinEdges = prctile(dxyvel(ind),[0:8:100]);
dct.spdBinCenters = mean(cat(1,dct.spdBinEdges(2:end),dct.spdBinEdges(1:end-1)));
dspdInds = discretize(dxyvel,dct.spdBinEdges);

tdphz = dphz(ind);
tdspdInds = dspdInds(ind);
tdErrlon = dErrlon(ind);
tdErrlat = dErrlat(ind);
tsmMask  = smMask(ind);
%figure();

shifts = 0:8:2^8;
for s = 1:numel(dct.spdBinCenters),
    for p = 1:numel(phzBinCenters),
        tic
        for k = 1:1;%numIter,
            sind = tdspdInds==s & tdphz==phzBinCenters(phzOrder(p));% & circshift(tsmMask, randsample(shifts,1));
            %sind(sind) = randn([sum(sind),1]) > 0;        
            mlon(s,p,k) = median(tdErrlon(sind));
            mlat(s,p,k) = median(tdErrlat(sind));
        end
        toc
    end
end
...
hold('on');
for p = 2:7,    
    plot(log10(dct.spdBinCenters(1:end-1)),sq(mlon(1:end-1,p,1)),'Color',clrs(p,:))
end
subplot(122);
hold('on');
for p = 2:7,    
    plot(log10(dct.spdBinCenters(1:end-1)),sq(mlat(1:end-1,p,1)),'Color',clrs(p,:))
end

figure,
hold('on');
for p = 2:7,    
    plot(sq(mlon(1:end-1,p,1)),sq(mlat(1:end-1,p,1)),'Color',clrs(p,:));
end
...
save('/storage/gravio/data/project/general/analysis/MjgER2016_decodeBhv_qntl.mat',...
     'qntl','pxp','npxp','mpxp','-v7.3');
load('/storage/gravio/data/project/general/analysis/MjgER2016_decodeBhv_qntl.mat');
