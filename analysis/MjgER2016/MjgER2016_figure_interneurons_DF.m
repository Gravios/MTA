...
% $$$           'PlotBoxAspectRatioMode','manual'};
% $$$ 
% $$$ binsDRe = linspace(-0.2,1,15);
% $$$ binsDRc = mean([binsDRe(2:end);binsDRe(1:end-1)]);
% $$$ 
% $$$ subplot(325);
...
% PLOT theta behaivor field
    [yind, yOffSet, xind, xOffSet] = deal(1, 0, 1+(e-1)*2, 0);
    sax(end+1) = axes(axOpts{:},                                            ...
                      'Position',[fig.page.xpos(xind)+xOffSet,              ...
                                  fig.page.ypos(yind)+yOffSet,              ...
                                  fig.subplot.width,                        ...
...
% PLOT theta place field
    [yind, yOffSet, xind, xOffSet] = deal(1, 0, 2+(e-1)*2, 0);
    sax(end+1) = axes(axOpts{:},                                            ...
                      'Position',[fig.page.xpos(xind)+xOffSet,              ...
                        fig.page.ypos(yind)+yOffSet,              ...
                        fig.subplot.width,                        ...
...
for  e = 1:numel(supExampleUnits),
    [yind, yOffSet, xind, xOffSet] = deal(sectionYind, 0, sectionXind+e*2-2, 0);
    sax(end+1) = axes(axOpts{:},                                                         ...
                      'Position', [fig.page.xpos(xind)+xOffSet,                          ...
                                   fig.page.ypos(yind)+yOffSet,                          ...
...
sectionYind = 5;
for  e = 1:numel(supExampleUnits),
    [yind, yOffSet, xind, xOffSet] = deal(sectionYind, -1, sectionXind+e*2-2, 0);
    sax(end+1) = axes(axOpts{:},                                                         ...
                      'Position', [fig.page.xpos(xind)+xOffSet,                          ...
                                   fig.page.ypos(yind)+yOffSet,                          ...
                                   fig.subplot.width*2+fig.subplot.horizontalPadding,    ...
...
sectionYind = 7;
for  e = 1:numel(supExampleUnits),
    [yind, yOffSet, xind, xOffSet] = deal(sectionYind, -2, sectionXind+e*2-2, 0);
    sax(end+1) = axes(axOpts{:},                                                         ...
                      'Position', [fig.page.xpos(xind)+xOffSet,                          ...
                                   fig.page.ypos(yind)+yOffSet,                          ...
                                   fig.subplot.width*2+fig.subplot.horizontalPadding,    ...
...
subplot2(2,3,1,2);
    ...
    xlim([0.5,3.5]);
subplot2(2,3,2,2);
    plot(rhoDRratio(30,:,5),'-+');
    xlim([0.5,3.5]);
subplot2(2,3,1,3);
    plot(dst,rho(189,:),'.');
    xlim([0.5,3.5]);
subplot2(2,3,2,3);
    plot(rhoDRratio(189,:,5),'-+')
    xlim([0.5,3.5]);
