...
% COMPUTE place fields and subsampled estimate
    ...
    for u = 1:numel(units),
        ...
        uRes = spk.res(spk.clu==unit);
        uSpk = spk.spk(spk.clu==unit,:,:);
        [~,sInd] = SelectPeriods(uRes,[stc{states{end},1}],'d',1,0);
        if numel(sInd)<=1, mspkt = zeros([size(uSpk,2),size(uSpk,3)]);
        else,             mspkt = bsxfun(@plus,sq(mean(uSpk(sInd,:,:)))',fliplr(1000:1000:8000));
        end
        ...
        [~,sInd] = SelectPeriods(uRes,[stc{states{end},1}],'d',1,0);
        if numel(sInd)>5,
            mspk = bsxfun(@plus,sq(mean(uSpk(sInd,:,:)))',fliplr(1000:1000:8000));
            plot(mspk,'b');
            sspk = sq(std(uSpk(sInd,:,:)))';
            plot(mspk-sspk,'r');
            plot(mspk+sspk,'r');
        end
        xlim([0,52]);
...
% WAVEFORM of unit
            ...
            hold('on');            
            [~,sInd] = SelectPeriods(uRes,[stc{states{s},1}],'d',1,0);
            if numel(sInd)>5,
                mspk = bsxfun(@plus,sq(mean(uSpk(sInd,:,:)))',fliplr(1000:1000:8000));
