...
% COMPUTE placefield centered rate scaled distance metric with approach/depart signature
    ghz = compute_ghz(Trial,unitSubset,pfts{t},'sampleRate',sampleRate,'sigma',sigma);
    ddz = compute_ddz(Trial,unitSubset,pfts{t},'sampleRate',sampleRate);
    drz = compute_drz(Trial,unitSubset,pfts{t},'sampleRate',sampleRate);
...
% $$$     for u = 1:numel(unitSubset),%&pfts{t}.data.spar>0.15&pfts{t}.data.spar<0.3),
% $$$         [mxr,mxp] = pfts{t}.maxRate(unitSubset(u));
...
% $$$                                       ['DRZ-',states{s},'-',num2str(numIter)],overwrite);
        [parmHRZall(ind,:,:,s), phzStatsHRZall(ind,:,:,s), rHRZall(ind,:,:,s), rhoHRZall(ind,:,:,s)] = ...
            MjgER2016_phasePrecession(Trial,ghz,ddz,phz,spkpp,unitSubset,[],[],numIter,...
