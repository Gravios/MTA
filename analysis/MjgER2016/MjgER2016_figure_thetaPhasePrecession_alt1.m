            ...
            if s == numStates-1,
                ...
                if ucnt == 2
                    ...
                    text(sum(sax(end).Position([1,3]))+0.2,sum(sax(end).Position([2,4]).*[1,0.5]),...
                         'Theta Phase','FontSize',8,'Color','k','VerticalAlignment','middle',...
...
% CREATE subplot axes
    ...
    text(2*pi-2,2*pi-0.5,(perm.states{s}),'HorizontalAlignment','center',...
...
[yind, yOffSet, xind, xOffSet] = deal(13, yOffSet, 1, 0);
for s = 1:3;
    
% ADJUST subplot coordinates       
    [yind, yOffSet, xind, xOffSet] = deal(yind, yOffSet, s+2, xOffSet+0.4);
...
%for g = 1:6, delete(sax(end));sax(end) = [];end
        sax(end+1) = axes('Units','centimeters',                                ...
                          'Position',[fig.page.xpos(xind)+xOffSet,              ...
        ...   y position          default height     shift   real height
        minus(fig.page.ypos(yind)+yOffSet,...
              [-double(find(o==otherStates)~=2)].*(fig.subplot.verticalPadding+ ...
              sax(end).Position(4))),                                           ...
                                      fig.subplot.width,                        ...
                                      numel(stind).*(2/40)],                    ...
                          'FontSize', 8,                                        ...
        ...
        sax(end).XTick = [];
        sax(end).YTick = [];        
        ylabel(sax(end),perm.states{o});
    end
    title(perm.states{s})
end    
        

% $$$         minus(fig.page.ypos(yind)+fig.subplot.height+yOffSet-numel(stind).*(2/40),...
% $$$               double(find(o==otherStates)~=1).*(fig.subplot.verticalPadding     ...
% $$$               +sax(end).Position(4)+0.2)),                                      ...
