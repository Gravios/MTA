...
exampleUnit(end).index = find( unitsEgo{exampleUnit(end).trialIndex} == exampleUnit(end).id);
egoCluSessionMapTrial = egoCluSessionMap( egoCluSessionMap(:,1) == exampleUnit(end).trialIndex, :);
exampleUnit(end).eindex = find( egoCluSessionMapTrial(:,2) == exampleUnit(end).id);
...
%%%<<< SUP EGO FIELD    --LOC--  partitioned by theta-phase and head-body-angle --
pc = bins.phz.count;
hc = bins.hba.count;
...
for untI = 1 : uc
    for phzI = 1 : pc
        for hbaI = 1 : hc
% COORDINATES 
            [yindex, yoffset] = deal( pc+1-phzI+(uc-1)*3-untI, -0.5  );
            [xindex, xoffset] = deal(    1+hbaI,  0    );
            [   gyo, gxo    ] = deal(         0,  0    );
            [yscale, xscale ] = deal(         1,  1    );
% SETUP         
...
% $$$                     rot90(egoHbaPhzRmaps_locSmooth.rmap(:, :, eindex, phzI, hbaI)',-1)) ...
% $$$                 .*egoHbaPhzRmaps_locSmooth.mask;
            xbins  = egoHbaPhzRmaps_loc.xpos-diff(egoHbaPhzRmaps_loc.xpos(1:2))/2;
            ybins  = egoHbaPhzRmaps_loc.ypos-diff(egoHbaPhzRmaps_loc.ypos(1:2))/2;
            ratemap = ...
                fliplr(...
                    rot90(egoHbaPhzRmaps_loc.rmap{etrial}(:, :, eindex, phzI, hbaI)',-1)) ...
                .*egoHbaPhzRmaps_loc.mask;
% PLOT 
            set(pcolor(xbins, ybins, ratemap),'EdgeColor','none');
% FORMAT 
            shading (sax,'flat');        
