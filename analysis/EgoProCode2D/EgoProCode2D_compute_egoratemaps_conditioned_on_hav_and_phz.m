...
for tind = 1:numel(Trials)
    ...
    rmap{tind} = nan([numel(xpos),numel(ypos),numel(units),phzN,hbaN]);
    %% cell array version
    for u = 1:numel(units)
        tic
    ...
    rmapShuff{tind} = nan([numel(xpos),numel(ypos),numel(units),phzN,hbaN,100]);
    %% cell array version
    for u = 1:numel(units)
        tic
...
load(fullfile(Trials{1}.path.project,'analysis','EgoProCode2D_compute_egoratemaps_conditioned_on_hba_and_phz_DATA.mat'));
