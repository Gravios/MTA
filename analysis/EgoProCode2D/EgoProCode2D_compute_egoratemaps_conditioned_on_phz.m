...
for tind = 1:numel(Trials)
    ...
    for u = 1:numel(unitSubset)
        ...
        for xind = 1:latticeSize(1)
            for yind = 1:latticeSize(2)
                ...
                mapHba{xind,yind} = discretize(thba(pind),bins.hba.edges([1,end]));
    ...
    %%%<<< permuted hba
% $$$ 
% $$$     rmapShuff{tind} = nan([numel(xpos),numel(ypos),numel(unitSubset),phzN,numel(bins.hba.centers),100]);
% $$$     %% cell array version
% $$$     for u = 1:numel(unitSubset)
% $$$         tic
% $$$ 
...
% $$$         for iter = 1:100
% $$$             tMHba = mapHba;
% $$$             occ = zeros([numel(mapOcc),phzN,numel(bins.hba.centers)]);
% $$$             for lind = 1:numel(mapOcc)
...
% $$$                     end
% $$$                 end
% $$$                 scc = zeros([numel(mapOcc),phzN,numel(bins.hba.centers)]);
% $$$                 for lind = 1:numel(mapOcc)
% $$$                     scc(lind,phzI,hbaI) = sum(mapW{lind}.*exp(-sum(mapSpk{lind}.^2,2)./(sigmaDS)),'omitnan');
% $$$                 end
...
save(fullfile(Trials{1}.path.project,...
              'analysis',...
              'EgoProCode2D_compute_egoratemaps_conditioned_on_phz_DATA.mat'),...
     'sampleRate',...
     'state',...
     'stcMode',...
     'bins',...
...
tind = 20;
u = find(unitsEgo{tind}==25);
figure,
for hbaI = 1%:hbaN
    for phzI = 1:phzN
        subplot2(phzN, ...
                 numel(bins.hba.centers),hbaN+1-phzI,hbaI);
        shading(gca(),'flat');
...
for hbaI = 1%:numel(bins.hba.centers)
    for phzI = 1:phzN
        subplot2(phzN,numel(bins.hba.centers),phzI,hbaI);
        shading(gca(),'flat');
        set(pcolor(xpos-diff(xpos(1:2))/2,ypos-diff(ypos(1:2))/2,fliplr(rot90(rmapShuff{tind}(:,:,u,phzI,hbaI,iter)',-1)).*mask),'EdgeColor','none');
        caxis([2,12])
        axis('xy');
        colormap('jet');
        colorbar();
