...
for tind = 1:numel(Trials)
    ...
    rmap{tind} = nan([numel(xpos),numel(ypos),numel(units),hbaN]);
    %% cell array version
    for u = 1:numel(units)
        tic
    ...
    rmapShuff{tind} = nan([numel(xpos),numel(ypos),numel(units),hbaN,100]);
    %% cell array version
    for u = 1:numel(units)
        tic
...
egoHbaRmaps = load(fullfile(Trials{1}.path.project,...
              'analysis',...
              'EgoProCode2D_compute_alloratemaps_conditioned_on_hba_DATA.mat'));
...
% $$$ shading(gca(),'flat');
% $$$ set(pcolor(xpos-diff(xpos(1:2))/2,ypos-diff(ypos(1:2))/2,fliplr(rot90(rmap(:,:,u,hbaI)',-1)).*mask),'EdgeColor','none');
% $$$ axis('xy');
% $$$ colormap('jet');
% $$$ colorbar();
