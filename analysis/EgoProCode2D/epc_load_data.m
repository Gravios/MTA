...
% >>> Prune Trials >>> --------------------------------------------------------
Trials = Trials(~cellfun(@isempty,Units));
...
for u = 1:numel(unitsEgo)
    egoCluSessionMap = ...
        cat(1,cluSessionMapHBA,[u*ones([numel(Units{u}),1]),Units{u}(:)]);
end
% EGOHVF 
cluSessionMapHVF = [];
for u = 1:numel(unitsEgo)
    egoHvfCluSessionMap = ...
        cat(1,cluSessionMapHVF,[u*ones([numel(UnitsHvf{u}),1]),UnitsEgoHvf{u}(:)]);
end
% <<< Unit Maps <<< -----------------------------------------------------------
