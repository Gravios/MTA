function [ixy,metadata] = compute_time_lagged_mutual_information(varargin)
...
FileName = fullfile(MTA_PROJECT_PATH,'analysis',['mis-time_lagged-',tag,'.mat']);
...
% SELECT time points of a specific transition type and cast to TimeSeries
        transIndex = cf(@(s,t,sts,f) get_state_transitions(s,t,sts,[],f), ...
                        ...
                        features);
        transIndex = cf(@(i)       round(mean(i,2))+30 ,  transIndex);
    ...
    for m = 1:size(v,2)
        for o = 1:size(v,2)
            ...
            for s = 1:numel(sbound)-1,
                ...
                fetVals = [v(ind,m),sv(ind,o)];
                [out,xb,yb,p]=hist2(fetVals(nniz(fetVals),:),edges,edges);
                pxy = out./nind;
                px = histc(v(ind,m),xb);
                px = px(1:end-1)/nind;
                py = histc(sv(ind,o),yb);
                py = py(1:end-1)/nind;
