...
for xshift = 1:numel(xshifts),
    for yshift = 1:numel(yshifts),
        medianPosError(xshift,yshift) = median(sqrt(sum([dc{cind}.ecom(ind,1)+xshifts(xshift),...
                                          dc{cind}.ecom(ind,2)+yshifts(yshift)].^2,2)));
    end
end
...
subplot(414);
    hold('on');
    plot(dc{cind}.ind(ind),dc{cind}.com(ind,4)*.8+0.1,'.r');
    plot(dc{cind}.ind(ind),dc{cind}.sax(ind,4)*.8+0.1,'.g');                
...
subplot2(3,2,1,1);
    ...
    hist2([dca.esax(ind,3),dca.phz(ind)],linspace(-1.5,1.5,40),phzBins)
subplot2(3,2,2,1);
...
subplot2(3,2,1,2);
    ind = dca.stcm(:,2)==2 & sind;
    hist2([dca.esax(ind,4),dca.phz(ind)],linspace(-1.5,1.5,40),phzBins)
subplot2(3,2,2,2);
    ind = (dca.stcm(:,3)==3|dca.stcm(:,4)==4) & sind;;
    hist2([dca.esax(ind,4),dca.phz(ind)],linspace(-1.5,1.5,40),phzBins)
subplot2(3,2,3,2);
