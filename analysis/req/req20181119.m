...
if ~exist('pfd','var'), [pfd,tags,eigVec,eigVar,eigScore,validDims,unitSubsets,unitIntersection,zrmMean,zrmStd] = req20180123_ver5(Trials,[],'13');  end
...
% $$$     eigVec = zeros([prod(pfd{1}.adata.binSizes),5]);
% $$$     for v = 1:5,    eigVec(vDims,v) = LR(:,v);    end
    %eigVec = reshape(eigVec,[pfd{1}.adata.binSizes',5]);    
    %eigVec = reshape(eigVec(:,3),[pfd{1}.adata.binSizes']);
...
uind = ismember(csmUnitSubset(:,1),[17:23]);

[~,csmPhzPrefMax] = max(sq(csmEigScrs(uind,1,:)),[],2);
csmPhzPrefMean = sum(reshape(bsxfun(@times,reshape(csmEigScrs,[],size(csmEigScrs,3)),exp(i.*pfs.adata.bins{2}')),size(csmEigScrs)),3)./sum(csmEigScrs,3);

figure,plot(sq(csmEigScrs(:,1,:))')
figure,hist(abs(csmPhzPrefMax(:,1)),linspace(0.5,16.5,16))
...
title(num2str(u));
%rmap = plot(pfs,u,'mean','colorbar',[],false,0.9);
...
betaPCR = regress(pmap(:,p)-mean(pmap(:,p)), PCAScores(:,1:2));
...
y = octane;
[n,p] = size(X);
[eigvec,scores,PCAVar] = pca(X,'Economy',false);
betaPCR = regress(y-mean(y), scores(:,1:2));
betaPCR = eigvec(:,1:2)*betaPCR;
...
[LU,LR,FSr,VT] = erpPCA(pmap');
betaPCR = regress(pmap(:,p)-mean(pmap(:,p)), LU(:,1:2));
betaPCR = LR(:,1:2)*betaPCR;
betaPCR = [mean(pmap(:,p)) - pmap(:,p)'*betaPCR; betaPCR]';
efit = [1; pmap(:,p)]*betaPCR;

efit = [ones([size(pmap,1),1]) pmap(:,p)]*betaPCR;
