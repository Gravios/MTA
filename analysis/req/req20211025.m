...
badUnits = cf(@(T) load(fullfile(T.spath,[T.name,'.bad_units.mat'])).badUnits, Trials);
...
hvfl = fet_href_HXY(Trials{t},sampleRate,[],'trb',1.5,r);
...
pft = pfs_2d_theta(Trials{t},units{t},[],false);
...
partitions = {{[-0.2,-1.5],@gt}, {[0.2,-0.2],@gt}, {[1.5,0.2],@gt};...
              {[-0.2,-1.5],@lt}, {[0.2,-0.2],@lt}, {[1.5,0.2],@lt}};
aper = tper & (pper|wper);

%% startfig
[hfig,fig,fax,sax] = set_figure_layout(figure(666013),'A4','portrait',[],3,3,0.1,0.1); 
...
sax(end+1) = axes('Units','centimeters',                                ...
                  ...
                  'LineWidth',1);
hold('on');
scatter3(fet(res,2),roll(res,1),ones([numel(res),1]), 5,pfsCenterHR(res,2),'filled');
caxis([-150,150]);
colormap('jet');
view([2])
xlim([-pi,pi]/2);
...
[yind, yOffSet, xind, xOffSet] = deal(3, -4, 1, 0);
% CREATE subplot axes
sax(end+1) = axes('Units','centimeters',                                ...
                  'Position',[fig.page.xpos(xind)+xOffSet+globalXOffset,...
                              fig.page.ypos(yind)+yOffSet+globalYOffset,...
...
for x = 1:3,for y = 1:2
    [yind, yOffSet, xind, xOffSet] = deal(y+2, 1, x+2, 0);
    sax(end+1) = axes('Units','centimeters',                                ...
                      'Position',[fig.page.xpos(xind)+xOffSet+globalXOffset,...
                                  fig.page.ypos(yind)+yOffSet+globalYOffset,...
                                  fig.subplot.width,                        ...
    ...
    if x==2 && y==1
        title('Pos of PFS Center in Head FOR for each Spike Time');
    end
    end
end
%%%>>>
...
%%%<<< PLOT spikes in headLat X headFwd space
for x = 1:3,for y = 1:2
    [yind, yOffSet, xind, xOffSet] = deal(y+4, 0, x+2, 0);
    sax(end+1) = axes('Units','centimeters',                                ...
                      'Position',[fig.page.xpos(xind)+xOffSet+globalXOffset,...
                                  fig.page.ypos(yind)+yOffSet+globalYOffset,...
                                  fig.subplot.width,                        ...
