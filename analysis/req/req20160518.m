...
FigDir = '/storage/gravio/ownCloud/MjgEd2016/figures/Figure_1/';

varargin = {};

[fetSet, mode, tag_preprocessing, tag_postprocessing, slist, sampleRate, nNeurons, nIter,...
 states, rndMethod, norm, mref,prctTrain] = DefaultArgs(varargin,defargs,'--struct');
...
for rli = 1:numel(slist),
    SesList = get_session_list(slist{rli});
    for sli = 1:numel(SesList),
        ...
        load(fullfile(MTASession().path.project,'analysis',...
...
for s = 1:2,
    SesList = get_session_list(slist{s});        
    for r = 1:2,
        if s==r, continue,end
        RefList = get_session_list(slist{r});
...
% print figure
print(hfig,'-depsc2',fullfile(FigDir,...
