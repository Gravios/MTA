...
% >>> Matching Template >>> ---------------------------------------------------
tid1 = find_trial_index(Trials,'jg05-20120325.cof.all');
tid2 = find_trial_index(Trials,'Ed10-20140815.cof.all');
...
Pfs = cf(@(T) pfs_2d_states(T,'all',[],states), Trials);
...
neuronSets = generate_neuron_sets(tmtch, cmtch, true);


figure,imagesc(cellfun(@numel,cmtch)')

for tind1 = 1:numel(Trials)
    for tind2 = 1:numel(Trials)
        cmtch{tind1}
        % A cell matrix where each element contains a pair of
        % string which indentify each Trial in the unit pairing
        % matrix with the name of each trial.
        %
        % A cell matrix where each element contains a matrix Nx4
        % matrix, where N is the number of pairs of units common
        % between the trials.
        %
        % Make a matlab script to generate sets of neurons which
        % are common across trials with the minimum set size of 2
        %
        % Generate a unit set struct
        %  - Each set is a cellarry 2xN
        %    - first row is the trial filebase: string
        %    - second row is the unit cluster id unique to that trail: number
        % For each set of units
        %  - check the second unit against the first unit of all
        %  other trials in that column. -> if a match is found
        %  append the second unit of the other trial to the current
        %  unit list
...
tind  = find_trial_index(Trials, 'jg05-20120309.cof.all'); 
cmtch{23}
pcount = [];
for tind = 1:numel(Trials);
    if ~isempty(cmtch{tind})
        if isnumeric(cmtch{tind})
            pcount(tind) = size(cmtch{tind},1);
        else
            pcount(tind) = sum(cellfun(@(c) size(c,1), cmtch{tind}));
        end
    end
end
...
tind = 10;
shnk = 3;
for units = cmtch{tind}{shnk}(:,1:2)'
    disp(units)
    subplot(1,2,1)
    plot(Pft{tind},units(1),[],'text');
    title(num2str(units(1)));
    subplot(1,2,2);
    plot(Pft{tind+1},units(2),[],'text');
    title(num2str(units(2)));
    waitforbuttonpress();
end
