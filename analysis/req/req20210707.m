...
for p = 1:24,
    ...
    msaxf(p) = median(dca.esax(ind,1),'omitnan');
end
figure,
subplot(211);
plot(phzBinc,msaxf);
...
% PLOT XY and est trajectories
t = 20;
ind = dca.sessionId==t & dca.ucnt>1;
figure();
subplot(211);hold('on');
plot(dca.ind(ind),dca.pos(ind,1),'.');
plot(dca.ind(ind),dca.com(ind,1),'.');
plot(dca.ind(ind),dca.sax(ind,1),'.');
subplot(212);hold('on');
plot(dca.ind(ind),dca.pos(ind,2),'.');
plot(dca.ind(ind),dca.com(ind,2),'.');
plot(dca.ind(ind),dca.sax(ind,2),'.');
linkaxes(findobj(gcf(),'Type','Axes'),'x');

Trial = Trials{t};
xyz = preproc_xyz(Trial,'trb');
xyz.resample(sampleRate);
for mind = 1:numel(xyz.model.Connections)
    xyz.model.Connections{mind}.color = [0,0,1];
end
...
%torig = 316200;
tshft = 325;
sind =   torig+tshft-12 < tind ...
       & torig+tshft+12 > tind ...
       & tucnt >1 ...
       & ismember(tiphz,4:20);
% $$$ thetaSegment = tphz(sind);
% $$$ tph = cell([1,2]);
% $$$ prevTph = thetaSegment(1);
% $$$ for ph = tphz(sind)';
plot(tsax(sind,1),tsax(sind,2));
scatter(tsax(sind,1),tsax(sind,2),20,tphz(sind),'Filled');
colormap('hsv');
caxis([0,2*pi]);
xyz.model.Markers{end}
plotSkeleton(Trial,...
             xyz,...
             torig+tshft,...
             'line',...
             ang,...
             {'spine_lower','pelvis_root','bcom','spine_middle',...
              'spine_upper','head_back'  ,'head_left'   ,...
              'head_front','head_right','hcom','nose'},...
             [-12,12] ...
             );
...
% LATERALIZATION of phase precession
est = 'esax'
figure()
sind =   dca.stcm(:,1) == 1                    ...
       & dca.stcm(:,2) ~= 2                    ...        
