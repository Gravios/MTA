...
for s = 1:numOfTrials
    ...
    hfig = figure(baseFigNum+s);                               % Create or select figure
    if s ==1,
        [fet,ftit,fdesc,fmean,fstd] = fet_tsne(Trial,[],true); % Load features and save means and stds
        stc = Trial.load('stc','hand_labeled_rev2');           % Load States, labeled by hand 
        clf                                                    % Clear the figure
    else
        [fet,ftit,fdesc,fmean,fstd] = fet_tsne(Trial);         % Load features with saved means and stds
    ...
    for f = 1:fet.size(2),
        for j = f+1:fet.size(2),
            ind = nniz(fet);                                   % Index for not zero, inf or nan           
            edgs    = {linspace([prctile(fet(ind,f),[2,98]),nbin])};% Create nbins over percentile 2 and 98 of fet
            edgs(2) = {linspace([prctile(fet(ind,j),[2,98]),nbin])};% Create nbins over percentile 2 and 98 of fet
            edc = edgs;
            ...
            contour(X,Y,o',[.0003,.0003],'linewidth',1.5,'Color',Scolors(s,:))
