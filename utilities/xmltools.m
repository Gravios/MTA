...
% Updates
%   - 20100923 (Nori): exclude parameters from "neuroscope" and "programs". Speeds up the code
...
while ~eot & ~isempty(udeblank(deblank(str)))
  ...
  % ----- begin Nori ---------------------------
  % don't read paramethers from neuroscope and programs
  f_end = strfind(str, '</');
  f_beg = strfind(str, '<');
  
  cut_beg = strfind(str, '<neuroscope');
  cut_end = strfind(str, '</programs>');
  
  if (~isempty(f_end) & ~isempty(f_beg) & ~isempty(cut_beg) & ~isempty(cut_end))
      id_beg = find(f_beg<cut_beg | f_beg>cut_end);
      f_beg = f_beg(id_beg);

      id_end = find(f_end<cut_beg | f_end>cut_end);
      f_end = f_end(id_end);  
  end
  % ----- end Nori ---------------------------
