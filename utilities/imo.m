    ...
    for i = 1:size(xyz,2),
        for j = 1:size(xyz,2),
            for k = 1:size(xyz,2),
                ...
                bs_ang(1,1) = atan2(norm(sq(ccr(:,i,j,k,:))),...
                ...
                for l = 1:size(xyz,2),
                    if length(unique([i,j,k,l]))==4,
                        ...
                        dang(1,1) = dot(sq(ccr(:,i,j,k,:)),...
                                        sq(markerDiffMat(:,i,l,:)),...
                                        find(size(sq(ccr(:,i,j,k,:)))==3));

                        cang(1,1,:) = cross(sq(ccr(:,i,j,k,:)),...
                                            sq(markerDiffMat(:,i,l,:)),...
                                            find(size(sq(ccr(:,i,j,k,:)))==3));

                        interMarkerOrientation(1,i,j,k,l,1) = atan2(norm(cang(1,1,:)),dang(1,1));

                        bs_projection = sq(cross(sq(ccr(:,i,j,k,:)),...
                                                 (sq(-cang)./ccr_mag))./ccr_mag);

                        dang(1,2) = dot(bs_projection,...
                                        sq(markerDiffMat(:,i,j,:)),...
                                        find(size(bs_projection)==3));                    

                        cang(1,2,:) = cross(bs_projection,...
                                            sq(markerDiffMat(:,i,j,:)),...
                                            find(size(bs_projection)==3));

                        dang(1,3) = dot(bs_projection,...
                                        sq(markerDiffMat(:,i,k,:)),...
                                        find(size(bs_projection)==3));                    

                        cang(1,3,:) = cross(bs_projection,...
                                            sq(markerDiffMat(:,i,k,:)),...
                                            find(size(bs_projection)==3));
            ...
            for k = 1:size(xyz,2),
                ...
                for l = 1:size(xyz,2),
                    if length(unique([i,j,k,l]))==4,
                        ...
                        cang(:,1,:) = cross(sq(ccr(:,i,j,k,:)),...
                                            sq(markerDiffMat(:,i,l,:)),...
                                            find(size(sq(ccr(:,i,j,k,:)))==3));

                        bs_projection = sq(cross(sq(ccr(:,i,j,k,:)),...
                                                 (sq(-cang(:,1,:))./ccr_mag))./ccr_mag);

                        dang(:,2) = dot(bs_projection,...
                                        sq(markerDiffMat(:,i,j,:)),...
                                        find(size(bs_projection)==3));                    

                        cang(:,2,:) = cross(bs_projection,...
                                            sq(markerDiffMat(:,i,j,:)),...
                                            find(size(bs_projection)==3));

                        dang(:,3) = dot(bs_projection,...
                                        sq(markerDiffMat(:,i,k,:)),...
                                        find(size(bs_projection)==3));                    

                        cang(:,3,:) = cross(bs_projection,...
                                            sq(markerDiffMat(:,i,k,:)),...
                                            find(size(bs_projection)==3));

                        bs_ang(:,1) = atan2(sqrt(sum(sq(ccr(:,i,j,k,:)).^2,2)),...
                                            dot(markerDiffMat(:,i,j,:),...
                                                markerDiffMat(:,i,k,:),4));
