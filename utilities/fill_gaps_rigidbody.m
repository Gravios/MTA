function xyz = fill_gaps_rigidbody(Session,varargin)
%function xyz = fill_gaps_rigidbody(xyz,rb_model,gind)
% 
...
[goodIndicies,rigidBodyMarkers,MarkerSwapErrorThreshold,bufferSize,periods] =                    ...
...
% LOAD Base session 
if ischar(Session) || isa(Session,'MTATrial'), Session = MTASession(Session); end
...
% SELECT good indicies with a gui if none are specified 
...
% COMUPUTE all marker solutions for each triad
...
% COMPUTE error of each triad solution
meanGoodErrorDist = [];
varGoodErrorDist  = [];
goodErrorGroupCount = [];
for i = unique(reid),
