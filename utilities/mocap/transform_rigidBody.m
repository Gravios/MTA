...
if display, 
%   DISPLAY figure to visualize the gradients generated by the tranformations
    hfig = figure(); 
end
...
% COMPUTE head speed
    vxy = xyz.vel('fhcom',[1,2]);
% RESTRICT computations to periods where head speed is greater than 4cm/s        
...
nxyz.addMarker('hcom', [0.5,1,0.5],{{'head_back','hcom',[0,0,1]}},xyz.com(rb));
nxyz.addMarker('head_neck',                       ... % Marker Name
               [1,0,1],                           ... % Marker Color
               {{'head_back','hcom',[1,0,1]}},    ... % Sticks Associated with Marker and Neighbors
               bsxfun(@plus,nx*mpoint(2)+ny*mpoint(1)+nz*mpoint(3),nxyz(:,'hcom',:)) ... XYZ Positions
);
