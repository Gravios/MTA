function [rbBasis, hcom] = compute_rb_basis_timeseries(xyz,varargin)
% function [rbBasis, hcom] = compute_rb_basis_timeseries(xyz,varargin)
%
% computes the basis of the rb specified by the markers centered on the mean 
...
ERR_PREFIX = 'MTA:utilities:mocap:compute_rb_basis_timeseries:';
...
rbModel = xyz.model.rb(markers);     % MTAModel

% COMPUTE the center of mass of the rigid body ∀ timepoints
hcom = xyz.com(rbModel);
...
rbBasis = cat(3,sq(nx),sq(ny),sq(nz));
