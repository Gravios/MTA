function Session = sync_ephys_vicon(Session,TTLValue,viconSampleRate)
%
...
%
% note - the event file must be located in the ephys folder
%
%
...
if ~exist(Session.spath,'dir')
    if ~exist(Session.spath,'dir') && ...
        ...
        e.message    = ['Session: ' Session.name ', cannot be found or does not exist. ' ...
                       ...
                       'for the MTA toolbox.'];
        e.identifier = 'MTA:utilities:sync_ephys_vicon:Session404';
        error(e);
    end
end
...
if isempty(viconSampleRate),
    ...
    if isempty(viconSampleRate),
        error('MTA:utilities:sync_ephys_vicon:emptySampleRate');
    end
else
    [xyzData, markers] = concatenate_vicon_files(Session);
end
% GENERATE marker model from vsk file (VICON IQ ONLY)
vsk_path = fullfile(Session.spath, ...
                    Session.maze.name, ...
                    [Session.name '-' Session.maze.name '.vsk']);
...
% THROW error if no EPHYS events match xyzData
assert(~isempty(viconSyncPeriods),ERR.type,ERR.msg,TTLValue);
% SELECT xyzData which match EPHYS events Pairs
