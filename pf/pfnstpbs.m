function pfnstpbs(Session,varargin)
...
place_grid = zeros(length(pax),length(pax),length(spkg),length(tau_limit),length(window_radius));
pred_grid = zeros(length(pax),length(pax),length(spkg),length(tau_limit),length(window_radius));
out_grid = zeros(length(pax),length(pax),length(spkg),length(tau_limit),length(window_radius));

for unit = 1:length(spkg)
    if isempty(spkg{unit})|length(myRes(myClu==unit))>30000,continue,end
    for tl = 1:length(tau_limit),
        futime = tau_limit(tl);
        trajdur = round(futime*Session.xyzSampleRate);
        futraj = GetSegs(sq(stsxyz(:,7,:)),round(myspk{unit}(spkg{unit}(:,4))/Session.lfpSampleRate*Session.xyzSampleRate+1),trajdur);
        for wr = 1:length(window_radius),
            ...
            for x = 1:length(pax),
                for y = 1:length(pax),
                    ...
                    place_grid(x,y,unit,tl,wr) = sum(tau==1)/length(tau);
                    pred_grid(x,y,unit,tl,wr) = sum(tau>1)/length(tau);
                    out_grid(x,y,unit,tl,wr) = sum(tau<0)/length(tau);
                end
            end
        end
    end
end
save([Session.spath.analysis Session.filebase 'pfnstp.rs.mat'],'place_grid','pred_grid','out_grid','nbins','pax','window_radius','tau_limit','unit')
