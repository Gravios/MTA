function [fet,featureTitles,featureDesc,Nmean,Nstd] = fet_tsne_rev7(Trial,varargin)
...
%% New zfet
name = 'lower spine pitch'; label = 'lsp'; key = 'z';
zv = MTADfet.encapsulate(Trial,...
                         circ_mean([ang(:,1,2,2),ang(:,1,3,2)],[],2),...
                         xyz.sampleRate,...
                         name,label,key);
...
ys.data = ys(:,round(fs)==5,1,1);
ys.resample(fxyz);
ys.data(~nniz(ys),:)=1e-20;
name = 'zv and wag pw'; label = 'zvwp'; key = 'w';
zfet = MTADfet.encapsulate(Trial,...
                         abs(log10(nanmedian(ys.data,2))),...
                         ys.sampleRate,...
                         name,label,key);


%% body smoothed distance psd 10hz
name = 'body smoothed distance'; label = 'bsd'; key = 'b';
bv = MTADfet.encapsulate(Trial,...
                         ang(:,'bcom','fubcom',3),...
                         xyz.sampleRate,...
                         name,label,key);

dspec = struct('nFFT',2^8,'Fs',bv.sampleRate,...
               'WinLength',2^7,'nOverlap',2^7*.875,...
               'FreqRange',[1,30]);
[ys,fs,ts] = fet_spec(Trial,bv,'mtchglong',false,[],dspec,false);

ys.data = ys(:,round(fs)==10,1,1);
ys.resample(fxyz);
ys.data(~nniz(ys),:)=1e-20;
name = 'body smoothed distanc psd 10hz'; label = 'bsd10'; key = 'p';
dfet = MTADfet.encapsulate(Trial,...
                         abs(log10(nanmedian(ys.data,2))),...
                         ys.sampleRate,...
                         name,label,key);
mxyz = xyz.copy;
mxyz.resample(newSampleRate);
ang = create(MTADang,Trial,mxyz);
