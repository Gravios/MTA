function [fet,featureTitles,featureDesc,Nmean,Nstd] = fet_bref_rev12(Trial,varargin)
...
fet.data = [ reshape(walkFetRot,size(xyz,1),[]),zvec,...
             log10(reshape(permute(rms(fldwalkFetRot.segs(circshift([1:size(fldwalkFetRot,1)]',20),40)),[2,3,4,1]),size(xyz,1),[])),...
             log10(reshape(permute(rms(fhdwalkFetRot.segs(circshift([1:size(fhdwalkFetRot,1)]',20),40)),[2,3,4,1]),size(xyz,1),[]))...
           ];
fet.data(~nniz(xyz),:)=0;
