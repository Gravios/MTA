...
function [fet,featureTitles,featureDesc] = fet_HPBPHS(Trial,varargin)
% function [fet,featureTitles,featureDesc] = fet_HPBPHS(Trial,varargin)
% {head-pitch body-pitch head-speed}
% varargin:
...
% MAP to reference trial
    ...
    xyz.filter('ButFilter',4,1.5);
    v = xyz.vel('hcom',[1,2]);
    v.data(v.data<0.001) = 0.001;
    v.data = log10(v.data);
% CONCATENATE features
    fet.data = [pch.data,v.data];
    fet.data(~nniz(xyz),:)=0;
    fet.save();
else
    load(fet,Trial);
