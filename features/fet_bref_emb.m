function [fet,featureTitles,featureDesc,Nmean,Nstd] = fet_bref_emb(Trial,varargin)
...
embeddingWindow = round(0.5*fet.sampleRate);
fet.data = fet.segs([],embeddingWindow);
fet.data = circshift(fet.data,round(embeddingWindow/2),2);
fet.data = reshape(permute(fet.data,[2,3,1]),size(fet,2),[]);
fet.data(~nniz(xyz),:)=0;
