function [fet] = fet_svd_mis(Trial,varargin)
...
[psFet] = decompose_xy_motion_wrt_body(Trial);
psFet.resample(xyz);
psFet.data(:,2) = abs(psFet.data(:,2));
...
if nargout>1,
    ...
    % 11
    featureTitles(end+1) = {'Forward Velocity'};
    featureDesc(end+1) = {['forward component of the decomposition of '...
                           'marker velocity with respect to the body']}

    %12
    featureTitles(end+1) = {'Absolute Lateral Velocity'};    
    featureDesc(end+1) = {['absolute lateral component of the decomposition of '...
                           'marker velocity with respect to the body']}
