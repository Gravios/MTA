function [fet,featureTitles,featureDesc,Nmean,Nstd] = fet_bref_SLPR(Trial,varargin)
...
% PREPROC xyz
xyz = Trial.load('xyz','trb');%preproc_xyz(Trial,procOpts);

hcom = xyz.com(xyz.model.rb({'head_back','head_left','head_front','head_right'}));
...
mvec = xyz(:,'hcom',[1,2])-xyz(:,'spine_lower',[1,2]);
...
for m = 1:numel(tmar),
    tvec(:,m,1:2) = xyz(:,tmar{m},[1,2])-mean([xyz(:,'spine_lower',[1,2]),xyz(:,'hcom',[1,2])],2);
