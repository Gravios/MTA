function [fet,featureTitles,featureDesc,Nmean,Nstd] = fet_bref_rev11(Trial,varargin)
...
defargs = struct('newSampleRate', Trial.xyz.sampleRate,    ...
                 'normalize'    , false,                   ...
                 'procOpts'     , {{'SPLINE_SPINE_HEAD_EQI_SMOOTH'}});
...
fet.data = [ reshape(walkFetRot,size(xyz,1),[]),zvec,...
             log10(reshape(permute(rms(fldwalkFetRot.segs(circshift([1:size(fldwalkFetRot,1)]',15),30)),[2,3,4,1]),size(xyz,1),[])),...
             log10(reshape(permute(rms(fhdwalkFetRot.segs(circshift([1:size(fhdwalkFetRot,1)]',15),30)),[2,3,4,1]),size(xyz,1),[])),...
             ang(:,'spine_lower','spine_upper',2),...
...
if normalize,
    [popMean,popStd] = load_normalization_parameters_unity(mfilename);
