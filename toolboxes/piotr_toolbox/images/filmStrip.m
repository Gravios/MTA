function F = filmStrip( I, overlap, delta, border, merge )
...
for i=1:nStrip
  ...
  Fi = -nan( sizF1 );  row = 1;  col = sizF1(2);
  for f=nFrame:-1:1
      if merge
          It =  I(:,:,:,f,i);
          It(It==1)=nan;
          Fit = Fi( row:(row+mRows-1), (col-nCols+1):col, : );
          Fit(Fit==1)=nan;
          Fi( row:(row+mRows-1), (col-nCols+1):col, : ) = nanmean(cat(4,Fit,It),4);
          if f==nFrame,
              Fit(~isnan(It)) = It(~isnan(It));
              Fi( row:(row+mRows-1), (col-nCols+1):col, : ) = Fit;
          end
      else
